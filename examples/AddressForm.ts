...
export const validator: FormValidator<AddressForm> = (
  ...
  if (parameters.zipCode === undefined && parameters.zipCode === '') {
    ...
    } else if (parameters.area !== undefined && parameters.area !== address.area) {
      errorMessages.area = '都道府県に誤りがないか確認してください'
    } else if (parameters.city !== undefined && parameters.city !== address.city) {
      errorMessages.city = '市区町村に誤りがないか確認してください'
    } else if (parameters.town !== undefined && parameters.town !== address.town) {
